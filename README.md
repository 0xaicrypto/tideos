...
| 路径 | 内容 | 承诺等级 |
|---|---|---|
| **主路径：原生 pkg + TideOS 商店** | 桌面应用（Firefox/Telegram/聊天/办公等 3 万+ ports） | ✅ 承诺稳定，原生性能 |
| **副路径：Linux 子系统（CLI/开发工具）** | `tideos-linux` 内 apt/brew | ✅ 承诺好用 |
| **第三路径：Wine** | Windows 应用 | ⚠️ 按需 |

用户心智一句话：**"图形的东西用商店装，命令行的东西两边都行。"**

## 终极愿景：装完就跑（Wine 式运行时接管）

**目标形态**：用户安装 Linux 桌面应用后直接运行，系统在后台自动调用 Linux 子系统作为运行时——就像 Wine 之于 Windows 应用：

```
用户操作：商店/apt 安装 Linux 桌面应用 → 双击启动 → 直接跑起来
系统行为：自动路由到 Linuxulator 运行时（binfmt 透明识别 + 显示层自动对接 + 桌面集成自动完成）
```
...
| 项 | 现状/路径 |
|---|---|
| 二进制透明执行 | ✅ Linuxulator 已具备（binfmt 自动识别 ELF） |
| 显示对接 | ⚠️ X11 可显示到 FreeBSD Xorg；Wayland 客户端实验性；GPU 加速未产品级 |
| 依赖系统 | ❌ systemd/Docker/Flatpak/Snap 不可用（Linuxulator 不支持） |
| 桌面集成 | 待建：启动器图标、`DISPLAY` 环境自动注入、GTK/Qt 主题桥接、文件对话框打通 |

**分阶段兑现**（不提前透支信任）：

1. **CLI 成熟**：`tideos-linux` 一键启用 + Ubuntu 用户态镜像 + brew/apt（开发工具生态）
2. **GUI 实验**：X11 桌面应用可运行（标注实验性），提供"应用启动器自动注入 DISPLAY"的雏形
3. **愿景层**：GPU 加速、Wayland、桌面集成完善后，达到"装完就跑、运行时自动接管"的 Wine 式体验

**Linux 子系统产品规范**：

- 默认关闭（体积/安全/性能），一键启用：加载 `linux64` 模块 + 拉取预构建用户态镜像（appx 式，避免在线安装失败率）
- 用户共享主目录（`mount_nullfs` 挂载 home，文件互通无感）
- 管理统一入口（TideOS 控制中心）：启用/停用/更新/重置
